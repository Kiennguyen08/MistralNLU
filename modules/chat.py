...
def get_generation_prompt(renderer, impersonate=False, strip_trailing_spaces=True):
    ...
    suffix_plus_prefix = prompt.split("<<|user-message-1|>>")[1].split(
        "<<|user-message-2|>>"
    )[0]
...
def generate_chat_prompt(user_input, state, **kwargs):
    ...
    instruct_renderer = partial(
        instruction_template.render, add_generation_prompt=False
    )
...
def load_character(character, name1, name2):
    ...
    if filepath is None or not filepath.exists():
        logger.error(
            f'Could not find the character "{character}" inside characters/. No character has been loaded.'
        )
        raise ValueError
...
def load_instruction_template(template):
    for filepath in [
        Path(f"instruction-templates/{template}.yaml"),
        Path("instruction-templates/Alpaca.yaml"),
    ]:
        if filepath.exists():
            break
    else:
